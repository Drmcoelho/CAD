...
| `perfis/` | `index.html` — fisiopatologia detalhada + tratamento dos 8 fenótipos de CAD, cada um com caso sintético calculado e link direto para abrir os números na calculadora do `app/`. Tem um **Tutor** no topo: digite os labs e `core/cad_core.js` (carregado direto, `window.CadCore`) classifica o perfil e refere à seção certa. Renderizado de `content/profiles.json`; cruzado com o `tratado/` (§1.6 e §5). |
