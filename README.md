...
| `content/` | bancos pedagógicos versionados (`questions`, `cases`, `atlas`, `profiles`, `gasometrias`, `fisiopatologia`), injetados nas saídas pelo `build_app.js` — nenhum conteúdo clínico hardcoded em HTML. |
| `app/` | `index.html` — o **núcleo interativo** (6 abas: Entender · Fisiologia · Calcular · Provão · Atlas · Referência). Calculadoras vivas, simuladores canvas, motor de questões. Provão tem duas fontes: `questions.json` (conduta) + `fisiopatologia.json` (mecanismo, questões ancoradas a `cad_core.js`/`abg_core.js` via campo `computed`). |
...
| `painel/` | `index.html` — painel de evolução gasométrica (série temporal de-identificada) + banco de gasometrias sintético para treino de raciocínio ácido-base (**100/100 casos completos**, incluindo quadros que não são CAD), claramente separado da série real. Motor: `core/abg_core.js` + `core/cad_core.js`. |
