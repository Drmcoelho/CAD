...
- [ ] Portão automático para a prosa livre de `atlas.banco`/`atlas.leis`/`questions.exp` — hoje só `POLICY` (deep-equal) e os 9 casos estruturados de `profiles.json` têm checagem mecânica; o furo acima sobreviveu justamente porque essas strings livres não são conferidas.
- [ ] Unificar o exercício hardcoded da calculadora "Osmolaridade efetiva" (`app/index.html`) com o banco B-6 do Atlas — mesmo conceito, duas fontes que já divergiram uma vez.
