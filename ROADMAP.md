...
- **Fase 4 — UI/render.** 🟡 (a) 🟡 `scripts/build_app.js` regenera o bloco canon a partir de `core.POLICY` (CANON = **artefato de build**, `--check` no CI barra edição à mão); injetar o **conteúdo pedagógico** (casos/questões) continua na Fase 3; (b) ✅ fallback textual + tabela para os 2 canvas (geradas do mesmo modelo, anti-drift); (c) 🟡 `scripts/smoke_app.js` — smoke **estrutural** browserless no CI (abas, bloco canon, tabelas, links); snapshot de **pixel** ainda aberto.
