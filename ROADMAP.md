...
- **Fase 3 — Dados.** 🟡 `content/questions.json` (banco do provão) e `content/cases.json` (casos socráticos A/B) extraídos do HTML; o app agora os lê de blocos `<script type="application/json">` **injetados pelo build** a partir das fontes (`build_app.js`), com `--check` no CI e regressão de DOM (idêntico ao pré-refatoração). Bancos secundários (cálculo, escada de leitura, fenótipos, leis, projetos) em `content/atlas.json`, mesmo mecanismo (bloco `atlas-data`). Aberto só: o build do DOCX lendo `content/*.json` (regeneração do DOCX pende de LibreOffice — ver STATUS).
